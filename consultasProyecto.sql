...
SELECT s.Proteina_Gen_idGen, s.sec FROM mydb.gen e, mydb.secuencia s 
WHERE s.longitud="137" AND s.Proteina_Gen_idGen= e.idGen;


SELECT s.sec, s.Proteina_entrada FROM mydb.gen a NATURAL JOIN mydb.secuencia s 
WHERE a.tipo="ndp" AND s.Proteina_Gen_idGen= a.idGen;
...
SELECT s.sec FROM mydb.secuencia s NATURAL JOIN mydb.proteina p WHERE s.proteina_entrada=p.entrada AND p.entrada=(
	SELECT po.Proteina_entrada FROM mydb.proteina_has_organismo po NATURAL JOIN mydb.proteina p 
    WHERE po.Organismo_nombre_cientifico="Homo sapiens" AND po.proteina_entrada=p.entrada);
    
SELECT Proteina_entrada, Proteina_Gen_idGen FROM mydb.Secuencia WHERE longitud<140 ORDER BY longitud;

ALTER TABLE mydb.secuencia ADD INDEX(longitud);
ALTER TABLE mydb.organismo ADD INDEX(tipo);
ALTER TABLE mydb.proteina ADD INDEX(nombre_entrada);
